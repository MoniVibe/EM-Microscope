...
describe("L6.6 Practical Study Workspace core", () => {
  it("shows executable, scaffold-only, and not-implemented capability statuses without scientific overclaim", () => {
    ...
    expect(capabilities.find((capability) => capability.id === "robust-design-advisor")?.status).toBe("executable");
    expect(capabilities.find((capability) => capability.id === "recenter-candidate-generation")?.status).toBe("executable");
    ...
    expect(capabilities.find((capability) => capability.id === "in-browser-1d-rcwa-preview")?.status).toBe("executable");
    expect(capabilities.find((capability) => capability.id === "rcwa-diffraction-order-table")?.status).toBe("executable");
    expect(capabilities.find((capability) => capability.id === "rcwa-harmonic-convergence")?.status).toBe("executable");
    expect(capabilities.find((capability) => capability.id === "rcwa-tmm-consistency")?.status).toBe("executable");
    ...
    expect(capabilities.find((capability) => capability.id === "2d-periodic-rcwa")?.status).toBe("not-implemented");
    expect(capabilities.find((capability) => capability.id === "production-rcwa-certification")?.status).toBe("not-implemented");
    expect(capabilities.find((capability) => capability.id === "anisotropic-rcwa")?.status).toBe("not-implemented");
    ...
    expect(markdown).toContain("In-browser 1D RCWA preview");
    expect(markdown).toContain("RCWA diffraction order table");
    expect(markdown).toContain("RCWA harmonic convergence diagnostics");
    expect(markdown).toContain("RCWA/TMM no-pattern consistency");
  ...
  it("saves and reimports a validation study bundle deterministically", () => {
    ...
    expect(imported.manifest.capabilityBoundary).toContain("L9.3 bounded diagnostic in-browser 1D periodic binary-grating RCWA");
    expect(imported.manifest.capabilityBoundary).toContain("certified validation");
    expect(imported.manifest.capabilityBoundary).toContain("production RCWA certification");
    expect(imported.manifest.capabilityBoundary).toContain("arbitrary 2D-periodic RCWA");
    expect(imported.manifest.capabilityBoundary).toContain("certified optical tolerancing");
    expect(imported.manifest.capabilityBoundary).toContain("production EM solver certification");
    expect(imported.manifest.capabilityBoundary).toContain("automatic final design approval");
    expect(imported.manifest.capabilityBoundary).toContain("auto redesign");
    expect(imported.manifest.capabilityBoundary).toContain("full inverse design");
