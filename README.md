...
An EM-first light simulator MVP. The visible web app is now the L10.0 Engineer Review Release Candidate front door plus the L9.9 Advisor Review Packet / Evidence Dossier Generator, L9.8 Guided Example Library / Known Experiment Pack, L9.7 Build My Simulation / Solver Method Decision Wizard, L9.6 Cross-Solver Consistency Bench, L9.5 Solver Router Evidence Auto-Pack, L9.4 Solver Router / Method Selection Matrix, L9.3 In-Browser 1D RCWA Preview Solver, L9.2 WebGPU-Accelerated 2D FDTD Sandbox, L9.1 Validation + Stability Harness, and L8.9 Real External FDTD Run Ingestion + Engineering Evidence Campaign Simulation Builder over the existing
Maxwell Design Foundry planar multilayer transfer-matrix workbench and L7.8 Detector Round-Trip Acceptance Pack /
Real Detector Bridge. L10.0 adds an engineer-review release-candidate package over the current L9.9-L7.x evidence stack: a top-level `Engineer Review / Release Candidate` mode, visible `L10.0 RC` badge, commit/build/public URL metadata, capability snapshot hash, example registry hash, evidence campaign hash, Full Current-State advisor packet hash, 15-minute/30-minute/full-evidence review paths, known limitations/capability truth rows, release smoke matrix, Lighthouse/public-quality guidance, and exports named `release_manifest.json`, `release_notes.md`, `review_checklist.md`, `known_limitations.md`, and `public_demo_script.md`. L10.0 is hardening and packaging only; it does not add solver physics, automatic correctness proof, certified validation, certified solver selection, production RCWA/FDTD certification, arbitrary 3D Maxwell, FEM/BEM, digital twin behavior, or manufacturing certification. L9.9 adds advisor review packet generation over the existing L9.8-L8.9 evidence stack. It lets users choose presets or selected examples/scenes, builds a claim ledger, checks evidence completeness, records expected-vs-computed/imported summaries, residual/convergence/consistency notes, unsupported gaps, and reproducibility receipts, and exports `advisor_packet.md`, `advisor_packet.json`, `advisor_packet_summary.csv`, `advisor_claim_ledger.csv`, `advisor_evidence_table.csv`, `advisor_gap_table.csv`, `advisor_review_questions.md`, `advisor_reproducibility_manifest.json`, and `advisor_packet.html`. Iteration count is not validation. This packet reports runnable evidence, references, residuals, convergence behavior, and limitations. L9.8 adds a searchable/filterable library of known starter examples for planar TMM, scalar diffraction, RCWA, 2D FDTD, external FDTD, camera/MTF diagnostics, evidence/robustness workflows, and unsupported gap reports. Each example preserves an id/hash, wizard answer set, solver route, expected physics, evidence task, limitations, campaign-promotion metadata, and exports named `example_report.md`, `example_report.json`, `example_registry.csv`, `example_scene_template.json`, and `example_wizard_answers.json` without adding solver physics or certifying a result. L9.7 guides Problem Type, Desired Output, Geometry, Materials, and Rigor/Evidence answers into deterministic L9.4 route decisions, L9.5 evidence tasks, L9.6 consistency cases, generated scene templates, unsupported gap reports, and exports named `simulation_decision_report.md`, `simulation_decision_report.json`, `simulation_decision_matrix.csv`, `generated_scene_template.json`, and `wizard_answers.json` without adding solver physics or certifying solver choice. L9.6 compares solver lanes only where their assumptions overlap: TMM vs no-pattern RCWA, CPU FDTD vs WebGPU/FDTD fallback parity, scalar aperture vs 2D FDTD diagnostics, TMM/Fresnel vs imported external FDTD slab evidence, absorber trend consistency, a missing-external-grating evidence slot, and a non-comparable TMM/scalar guardrail. It reports `PASS`, `WARNING`, `FAIL`, `NOT COMPARABLE`, and `NEEDS EXTERNAL EVIDENCE`; preserves L9.5 evidence task hashes; and exports `cross_solver_consistency_report.md`, `cross_solver_consistency_report.json`, `consistency_metrics.csv`, `solver_pair_residuals.csv`, and `consistency_assumptions.csv` without claiming solver correctness. L9.5 turns each L9.4 route decision into deterministic evidence tasks for TMM, scalar propagation, RCWA convergence, 2D FDTD validation, external FDTD run packs, or unsupported-gap reporting. It preserves route/task/scene hashes; lists required inputs, generated artifacts, route-specific actions, warnings, and validation rows; exports `solver_evidence_task.md`, `solver_evidence_task.json`, `solver_evidence_artifacts.csv`, and `solver_evidence_validation_plan.csv`; and promotes task metadata into the Engineering Evidence Campaign handoff without claiming solver correctness. L9.4 adds a deterministic method-selection layer that classifies planar stacks, ideal scalar aperture/lens chains, 1D periodic gratings, bounded 2D FDTD slices, finite external-FDTD geometry, and unsupported/scaffold scenes. It recommends PlanarTmmBackend, scalar propagation, RCWA Preview, 2D FDTD CPU/WebGPU diagnostics, External FDTD / Meep export-import, or unsupported/scaffold; explains accepted/rejected solvers; lists assumptions, limitations, validation checks, cost/risk, and route actions; renders a Method Selection Matrix; and exports `solver_route_report.md`, `solver_route_report.json`, `solver_route_matrix.csv`, `unsupported_items.csv`, and `validation_plan.csv`. L9.3 adds a bounded browser-native 1D periodic RCWA/Fourier-modal preview lane for simple binary rectangular gratings and patterned single layers. It computes reflected/transmitted diffraction order tables, order angles and propagating/evanescent/near-cutoff status, R/T/A totals, energy-balance checks, harmonic convergence sweeps, no-pattern consistency against the existing planar TMM backend, and exports for `rcwa_report.md`, `rcwa_report.json`, `rcwa_orders.csv`, `rcwa_convergence.csv`, and `rcwa_tmm_consistency.csv`. L9.2 keeps the capped browser-native 2D TMz FDTD sandbox with CPU reference Ez/Hx/Hy typed-array stepping, optional WebGPU acceleration when browser/secure-context/adapter/device/memory guardrails permit it, grid/object/step/monitor caps, fixtures, field/intensity/material views, Simulation Builder 2D slice handoff, visible CFL/dt/grid stability diagnostics, NaN/Infinity guards, boundary proximity warnings, Fresnel/absorber/symmetry reference checks, bounded grid-convergence diagnostics, CPU/GPU parity checks, performance diagnostics, and exports for `fdtd2d_validation_report.md`, `fdtd2d_validation_report.json`, `fdtd2d_backend_report.md`, `fdtd2d_backend_report.json`, `fdtd2d_parity.csv`, `fdtd2d_performance.csv`, `fdtd2d_convergence.csv`, `fdtd2d_stability_report.json`, `fdtd2d_energy_trace.csv`, `fdtd2d_monitor_trace.csv`, plus the L9.0-compatible sandbox reports/traces. L8.9 keeps the ordered Grid -> Source -> Elements -> Target / Material -> Compute -> Validate
...
L9.4 is a deterministic solver-router and method-selection layer only: it recommends a solver lane, explains assumptions and rejections, and exports route evidence. It is not automatic solver correctness proof, not automatic certified solver selection, not arbitrary 3D Maxwell, not FEM/BEM, not production RCWA/FDTD certification, not an external solver replacement, not digital twin behavior, and not manufacturing certification.
L9.5 is a deterministic evidence task auto-pack only: it packages route evidence, artifacts, validation plans, external run instructions, import/receipt checklists, unsupported-gap reports, and campaign promotion metadata. It is not a new solver, not automatic correctness proof, not automatic certified solver selection, not production RCWA/FDTD certification, not arbitrary 3D Maxwell, not FEM/BEM, not an external solver runner, not digital twin behavior, and not manufacturing certification.
L9.6 is a deterministic cross-solver consistency bench only: it compares overlap cases, labels non-overlap and missing-evidence cases, preserves evidence hashes, and exports residual/assumption reports. It is not a new solver, not automatic correctness proof, not automatic certified solver selection, not certified validation, not production RCWA/FDTD certification, not arbitrary 3D Maxwell, not FEM/BEM, not an external solver runner, not digital twin behavior, and not manufacturing certification.
L9.7 is a deterministic simulation intake wizard only: it maps user intent to existing solver routes, templates, evidence actions, and gap reports. It is not a new solver, not automatic correctness proof, not automatic certified solver selection, not production RCWA/FDTD certification, not arbitrary 3D Maxwell, not FEM/BEM, not an external solver runner, not digital twin behavior, and not manufacturing certification.
L9.8 is a deterministic guided example library only: it maps known experiment starter cards to existing L9.7 wizard answers, L9.4 routes, L9.5 evidence tasks, L9.6 consistency cases, RCWA/FDTD handoffs, diagnostics, exports, and unsupported gap reports. It is not a new solver, not automatic correctness proof, not certified validation, not certified solver selection, not production RCWA/FDTD certification, not arbitrary 3D Maxwell, not FEM/BEM, not a digital twin, and not manufacturing certification.
L9.9 is a deterministic advisor review packet generator only: it packages selected examples/scenes, solver routes, evidence tasks, claim ledger rows, completeness warnings, gap tables, and receipts into review exports. It is not a new solver, not automatic correctness proof, not certified validation, not certified solver selection, not production RCWA/FDTD certification, not arbitrary 3D Maxwell, not FEM/BEM, not a digital twin, and not manufacturing certification.
L10.0 is a deterministic engineer-review release-candidate package only: it organizes release metadata, review paths, known limitations, smoke evidence, advisor packet exports, release notes, review checklists, and public-quality guidance over the existing L9.9-L7.x workflows. It is not a new solver, not automatic correctness proof, not certified validation, not certified solver selection, not production RCWA/FDTD certification, not arbitrary 3D Maxwell, not FEM/BEM, not an external solver replacement, not a digital twin, and not manufacturing certification.
...
- `L10.0 Engineer Review Release Candidate`: the default top-level front door for engineer review and release-candidate packaging. It shows the `L10.0 RC` release badge, commit/build/public URL metadata, capability snapshot hash, L9.8 example registry hash, L8.8 evidence campaign hash, L9.9 Full Current-State advisor packet hash, 15-minute/30-minute/full-evidence review paths, implemented/diagnostic/external/scaffold/not-implemented capability truth rows, release smoke matrix, Lighthouse/public-quality instructions, and exports named `release_manifest.json`, `release_notes.md`, `review_checklist.md`, `known_limitations.md`, and `public_demo_script.md`. It is release hardening and review packaging only; no new solver physics, automatic correctness proof, certified validation, certified solver selection, production RCWA/FDTD certification, arbitrary 3D Maxwell, FEM/BEM, digital twins, or manufacturing certification are claimed.

- `L9.9 Advisor Review Packet / Evidence Dossier Generator`: the advisor-packet mode for engineering review packets. It builds Physics Sanity, Surface Geometry, Solver Credibility, Workflow, Full Current-State, or custom packets from L9.8 examples and L9.x workflow evidence; reports selected scenes, solver routes, evidence tasks, claim ledger rows, expected-vs-computed/imported summaries, residual/convergence/consistency notes, unsupported gaps, completeness checks, review questions, and reproducibility receipts; and exports `advisor_packet.md`, `advisor_packet.json`, `advisor_packet_summary.csv`, `advisor_claim_ledger.csv`, `advisor_evidence_table.csv`, `advisor_gap_table.csv`, `advisor_review_questions.md`, `advisor_reproducibility_manifest.json`, and `advisor_packet.html`. It is review/reporting workflow only; automatic correctness proof, certified validation, certified solver selection, production RCWA/FDTD certification, arbitrary 3D Maxwell, FEM/BEM, digital twins, and manufacturing certification stay out of scope.

- `L9.8 Guided Example Library / Known Experiment Pack`: the default top-level front door for known starter examples. It searches and filters examples by solver lane, physics type, difficulty, browser-runnable status, external evidence, analytic reference, convergence evidence, measured-data workflow, and unsupported/gap status; loads examples into L9.7 wizard answers, RCWA Preview, the 2D FDTD sandbox, diagnostic workbenches, engineering-evidence handoffs, or unsupported gap reports; preserves example id/hash plus route/evidence hashes; and exports `example_report.md`, `example_report.json`, `example_registry.csv`, `example_scene_template.json`, and `example_wizard_answers.json`. It is a starter workflow and evidence-index layer only; automatic correctness proof, certified validation, certified solver selection, production RCWA/FDTD certification, arbitrary 3D Maxwell, FEM/BEM, digital twins, and manufacturing certification stay out of scope.

- `L9.7 Build My Simulation / Solver Method Decision Wizard`: the guided simulation intake layer under the example library. It asks for Problem Type, Desired Output, Geometry, Materials, and Rigor / Evidence Level; recommends PlanarTmmBackend, scalar propagation, RCWA Preview, bounded 2D FDTD diagnostics, External FDTD / Meep export-import evidence, or unsupported gap reporting; creates scene templates or sandbox handoffs; links to L9.5 evidence packs and L9.6 consistency checks; and exports `simulation_decision_report.md`, `simulation_decision_report.json`, `simulation_decision_matrix.csv`, `generated_scene_template.json`, and `wizard_answers.json`. It is guidance and evidence packaging only; automatic correctness proof, certified solver selection, production RCWA/FDTD certification, arbitrary 3D Maxwell, FEM/BEM, external-solver replacement, digital twins, and manufacturing certification stay out of scope.

- `L9.6 Cross-Solver Consistency Bench`: a Simulation Builder panel that compares only overlapping solver lanes with deterministic residuals and status categories. It includes TMM vs RCWA no-pattern consistency, CPU FDTD vs WebGPU FDTD parity, scalar aperture vs 2D FDTD diagnostics, TMM/Fresnel vs imported external FDTD slab evidence, absorber consistency, a missing RCWA-vs-external-grating evidence slot, and a TMM-vs-scalar non-comparable guardrail. It shows assumptions, warnings, required evidence, case hashes, preserved L9.5 task hashes, and exports named `cross_solver_consistency_report.md`, `cross_solver_consistency_report.json`, `consistency_metrics.csv`, `solver_pair_residuals.csv`, and `consistency_assumptions.csv`. It is a consistency diagnostic only; automatic correctness proof, certified solver selection, production RCWA/FDTD certification, arbitrary 3D Maxwell, FEM/BEM, external-solver replacement, digital twins, and manufacturing certification stay out of scope.

- `L9.5 Solver Router Evidence Auto-Pack / L9.4 Method Selection Matrix`: a Simulation Builder panel that classifies the current builder scene or deterministic examples for planar coatings, ideal aperture/lens scalar chains, binary gratings, bounded 2D finite slices, finite external-FDTD geometry, and unsupported curved/freeform material lenses. It shows the recommended solver, available alternatives, accepted/rejected reasons, assumptions, limitations, validation checks, cost/risk, route-specific actions, unsupported items, and a clickable Method Selection Matrix. It also generates route-specific evidence packs for TMM, scalar, RCWA, 2D FDTD, external FDTD, and unsupported routes, including task hashes, scene hashes, generated artifacts, validation rows, external run-pack/import actions, promotion metadata, and exports named `solver_evidence_task.md`, `solver_evidence_task.json`, `solver_evidence_artifacts.csv`, and `solver_evidence_validation_plan.csv`. L9.4 route exports remain named `solver_route_report.md`, `solver_route_report.json`, `solver_route_matrix.csv`, `unsupported_items.csv`, and `validation_plan.csv`. It is evidence packaging and method recommendation only; automatic correctness proof, certified solver selection, arbitrary 3D Maxwell, FEM/BEM, production RCWA/FDTD certification, external-solver replacement, digital twins, and manufacturing certification stay out of scope.
...
- `L9.7 Simulation Builder + Decision Wizard + Cross-Solver Consistency + Evidence Auto-Pack`: a top-level Simulation Builder workflow that
