...
Not scope:
- No automatic certified solver selection or solver correctness proof; L9.8 is a guided example starter library only, L9.7 is guided intake only, L9.6 is cross-solver consistency diagnostics only, L9.5 is evidence task packaging, and L9.4 is method recommendation only.
...
- No FEM/BEM route execution, arbitrary 3D Maxwell route execution, external solver replacement, digital twin, or manufacturing certification.
...
`l94_browser_smoke_code.js`

Playwright CLI smoke helper for L9.4 Solver Router / Method Selection Matrix: planar/TMM, scalar, RCWA, 2D FDTD, external FDTD, unsupported/scaffold routes, matrix explanations, route report export filenames, and L9.3/L9.2/L8.9/L8.8 regression visibility:

```powershell
npx --yes --package @playwright/cli playwright-cli open https://monivibe.github.io/EM-Microscope/
npx --yes --package @playwright/cli playwright-cli run-code --filename tools/fdtd/l94_browser_smoke_code.js
```

`l95_browser_smoke_code.js`

Playwright CLI smoke helper for L9.5 Solver Router Evidence Auto-Pack: deterministic evidence task generation, route-specific TMM/scalar/RCWA/2D FDTD/external FDTD/unsupported artifacts, export filenames, external run-pack actions, campaign promotion metadata, and L9.4/L9.3/L9.2/L8.9/L8.8 regression visibility:
...
`l97_browser_smoke_code.js`

Playwright CLI smoke helper for L9.7 Build My Simulation / Solver Method Decision Wizard: guided intake answers, route recommendations, scene-template creation, evidence pack generation, consistency-bench linking, unsupported gap reporting, decision export filenames, and L9.6/L9.5/L9.4/L9.3/L9.2 regression visibility:

```powershell
npx --yes --package @playwright/cli playwright-cli open https://monivibe.github.io/EM-Microscope/
npx --yes --package @playwright/cli playwright-cli run-code --filename tools/fdtd/l97_browser_smoke_code.js
```

`l98_browser_smoke_code.js`

Playwright CLI smoke helper for L9.8 Guided Example Library / Known Experiment Pack: known experiment search/filter, Airy/scalar route, RCWA route/action, 2D FDTD sandbox handoff, external FDTD run-pack example, unsupported gap report, evidence pack generation, Engineering Evidence Campaign handoff metadata, example report exports, and L9.7/L9.3/L9.2 regression visibility:

```powershell
npx --yes --package @playwright/cli playwright-cli open https://monivibe.github.io/EM-Microscope/
npx --yes --package @playwright/cli playwright-cli run-code --filename tools/fdtd/l98_browser_smoke_code.js
```
